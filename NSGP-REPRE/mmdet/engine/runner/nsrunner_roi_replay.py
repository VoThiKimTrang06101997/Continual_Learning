...
class BRNullSpaceRunner(Runner):
    def __init__(
        ...
        # ns_thresh
        self.ns_thresh = cfg.get('ns_thresh') if cfg.get('ns_thresh') != None else 0.5
        self.ns_init = cfg.get('ns_init') if cfg.get('ns_init') else False
    ...
    def train(self) -> nn.Module:
        ...
        if self.task_id != 1:
            ...
            print("===================================================")
            print("Be aware!! Resume from trained model are not allowed!")
            print("===================================================")
            # Teacher is initlized from loaded model. Resume from trained model change the teacher to the resumed half trained model.
        ...
        if not self.is_trained or True:
            self._has_loaded = False
            # EWC for BN, or you can freeze all BN, it doesn`t matter. And my EWC is not carefully tuned as the model is not very sensitive to it.
    ...
    def test(self) -> dict:
        ...
        self.call_hook('after_run')
        # Just in case you want to recompute the fea in and importance for Null Space.
        self.cal_fea_in(self.train_dataloader)
        self.calculate_save_importance(self.train_dataloader)
        return metrics
    ...
    def update_optim_transforms(self, train_loader):
        ...
        self.optim_wrapper.optimizer.get_eigens(self.fea_in, distinguisher = f"_{type(self.optim_wrapper.optimizer).__name__}_{len(self.ignore_keys)}_{self.ns_thresh}")
        # self.logger.info(f"** {self.ns_thresh} reserved {count} psudo samples, {reserved_area}/{total_area}={reserved_area * 100 / total_area}% area was reserved in covariance.")
    ...
    def cal_fea_in(self, train_loader):
        ...
        for i, data_batch in tqdm(enumerate(train_loader), desc="feain"+str(get_rank()), disable=False):
            if i > max_iter:
                break
    ...
    def cal_rois(self, train_loader):
        ...
        if self.task_id != 1:
            ...
            print("Calculating drifts.... ", hasattr(self, "olds"))
            # do correction
            if hasattr(self, "olds"):
                print("Calculating drifts....")
                self.projector.eval()
                cls_centers = []
                distances = []
                for i in tqdm(range(self.task_split[self.task_id-1]), disable=True):
                    cls_mask = cls_targets == i
                    cls_center = bbox_featss[cls_mask].mean(dim=0, keepdim=True)
                    distance = ((bbox_featss[cls_mask].reshape(-1, 256*7*7) - cls_center.reshape(-1, 256*7*7))**2).sum(dim=-1)
                    distances.append(distance)
                    cls_centers.append(cls_center)
                    # print(f"class {i}, {torch.isfinite(cls_center).all()}")
                cls_centers = torch.cat(cls_centers, dim=0).reshape(-1, 256*7*7)
                sigma = torch.cat(distances).std()
                # gap = self.displacement(self.olds.reshape(-1, 256*7*7), self.news.reshape(-1, 256*7*7), cls_centers.reshape(-1, 256*7*7), sigma=sigma)
                gap = self.projector(cls_centers) - cls_centers
                # torch.save([self.olds, self.news, self.cus_labels], osp.join(self.work_dir, 'old_new.pth'))
                for i, cls_idx in enumerate(cls_targets):
                    # print(bbox_featss.shape, gap.shape)
                    bbox_featss[i] = bbox_featss[i] + gap[cls_idx].to(bbox_featss.device).reshape(-1, 256, 7, 7)
            
            # bbox_featss, cls_targets, cls_weights, bbox_targets, bbox_weights, roiss = bbox_featss.to(tmp_device), cls_targets.to(tmp_device), cls_weights.to(tmp_device), bbox_targets.to(tmp_device), bbox_weights.to(tmp_device), roiss.to(tmp_device)
    ...
    def calculate_save_importance(self, dataloader):
        ...
        for idx, data_batch in tqdm(enumerate(train_dataloader), disable=False, desc="importance"+str(get_rank())):
            if idx > max_iter:
                break
            # with self.optim_wrapper.optim_context(self):
