...
class FasterRCNNRoIReplay(TwoStageDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        with torch.no_grad():
            if hasattr(self, "teacher_model"):
                ...
                if isinstance(self.teacher_model.roi_head.bbox_head, list):
                    for i in range(3): # This is preserved for Cascade RCNN, which has 3 roi head. 
                        self.teacher_model.roi_head.bbox_head[i].task_id = self.teacher_model.roi_head.bbox_head[i].task_id - 1
                else:
    ...
    def forward(self,
        ...
        if mode == 'loss':
            ...
            return self.get_bbox_stuff(inputs, data_samples)
        else:
